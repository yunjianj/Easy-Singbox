...
> **依赖分级（v1.3.2 起）**：`curl` / `openssl` 为**必需**（缺失会中止安装）；`ss`（iproute2）与 `iptables`/`nftables` 为**可选**，缺失只降级不阻断——
...
**证书复用与重签**：
- 卸载时若保留 acme.sh 账户，重装同一域名会**自动复用**既有有效证书（不再重复签发、不报错）；换域名或无有效证书时才真正发起签发。
- 需要**强制重新签发**（如怀疑证书被吊销、想更换密钥）时，用主页面选项 3「变更证书配置」→ 输入同域名并选择验证方式，脚本会带 `--force` 重签。

## 端口开放三选一

安装时会询问端口开放策略（直接回车默认选 1）：

1. 全部开放（22/SSH + 80 + 已启用协议端口；Hy2 跳跃段由 REDIRECT 自动转发到基础端口）
...
安装完成（或选「查看节点」）后，终端显示已启用协议的 URI，并写入 `/etc/sing-box/nodes.txt`。
...
- **SOCKS5**：`socks5://user:pass@host:port`（带认证）或 `socks5://host:port`（未设认证）。终端会额外输出 sing-box outbound JSON 兜底。
